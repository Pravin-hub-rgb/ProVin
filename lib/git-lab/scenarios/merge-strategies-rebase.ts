...
export const MERGE_STRATEGIES_REBASE: Scenario = {
  id: "merge-strategies-rebase",
  phase: "2.5.3",
  title: "Rebase Merge Strategy",
  description: "Create a feature branch, stage and commit your changes, push to GitHub, then merge using the rebase and merge strategy that replays commits with new hashes.",
  setup: (state) => {
    const now = Date.now()
    const initCommit = {
      hash: "a1b2c3d",
  ...
  steps: [
    ...
    },
    {
      ...
      hints: [
        "Open a PR so you can merge your feature branch using the rebase strategy.",
    ...
    {
      actor: "A",
      instruction: "Merge the PR using 'Rebase and merge' — your commit gets a new hash on main",
      match: (p) => p.type === "pr-merge" && p.strategy === "rebase",
      githubAction: "merge-pr",
      hints: [
        "Click the Merge Pull Request button. In the dropdown, pick 'Rebase and merge'.",
        "Rebase replays your commit onto main with a new hash. No merge commit — linear history.",
        "Select 'Rebase and merge', then click Merge Pull Request and Confirm.",
      ],
    },
    {
      actor: "A",
      instruction: "View the log to see the reapplied commit with a new hash",
      match: (p) => p.type === "log",
      hints: [
        "Your commit is on main with a NEW hash. No merge commit — just a straight line.",
        "Use git log and look at the hash value. It'll be different from when you first committed.",
        "Run: git log",
      ],
    },
  ],
