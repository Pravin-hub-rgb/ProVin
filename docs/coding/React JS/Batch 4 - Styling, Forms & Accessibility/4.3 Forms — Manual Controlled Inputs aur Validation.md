...
`React.SyntheticEvent<HTMLFormElement>` — TypeScript ko bata rahe hain: "yeh form submit ka event hai — `HTMLFormElement` type ke form se aaya hai." Ab `e.preventDefault()` pe koi error nahi.
...
Jab inline likhte hain — TypeScript context se samajh jaata hai ki yeh form ka submit event hai. `onSubmit` ka type already React mein defined hai — usse `e` ka type automatically mil jata hai. `React.SyntheticEvent<HTMLFormElement>` — wahi type hota hai andar se.
